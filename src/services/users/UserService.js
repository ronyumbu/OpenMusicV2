...
// menambahkan user
    async addUser({username, password, fullname}) {
        ...
        if (!rows[0].id) {
            throw new InvariantError('User tidak dapat ditambahkan');
        };
        return rows[0].id;
    }

// melakukan verifikasi terhadap user kredensial
...
// melakukan verifikasi username
    async verifyUsername(username) {
        ...
        if (rowCount > 0) {
            throw new InvariantError('Gagal menambahkan user karena username sudah digunakan');
        };
    }

// melakukan verifikasi terhadap user
